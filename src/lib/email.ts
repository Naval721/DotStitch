...
export const sendOTPEmail = async (email: string, otpCode: string): Promise<boolean> => {
  try {
    ...
    const emailOptions: EmailOptions = {
      to: email,
      subject: 'Verify Your Email - DotStitch',
  ...
  return `
    ...
    <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
      <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
        <h1 style="color: white; margin: 0;">DotStitch</h1>
      ...
      <div style="background: #f9f9f9; padding: 30px; border-radius: 0 0 10px 10px;">
        ...
        <p>Thank you for signing up with DotStitch! To complete your registration, please verify your email address using the code below:</p>
        ...
        <p style="color: #999; font-size: 12px; text-align: center;">
          © 2025 DotStitch. All rights reserved.
        </p>
      </div>
    </body>
    </html>
...
const generateOTPEmailText = (otpCode: string): string => {
  return `
Verify Your Email - DotStitch

Thank you for signing up with DotStitch!
